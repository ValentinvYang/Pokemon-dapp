...
export async function startHeliaServer(port = 8080) {
  const helia = await createHelia();
  const fs = unixfs(helia);

  const server = createServer(async (req, res) => {
    // SAME CODE AS helia-server.js
  ...
  return new Promise((resolve) => {
    server.listen(port, () => {
      console.log(`🚀 Helia test server running on http://localhost:${port}`);
      resolve({ helia, fs, server });
    });
  });
}
